'use babel'

import * as util from './util'
import { Range } from 'atom'

export default {
  wordRegExp: /([\w\.\-_]+)/,
  providerName: "mope-modelica",
  setClient(client) {
    this.client = client
    console.log("setting client in hyperclickProvider")
  },

  getSuggestionForWord(editor, text, range) {
    const self = this
    return new Promise((resolve, reject) => {
  	  if(util.isModelicaFile(editor.getPath()) && text.trim() !== "") {
        const className = text
        self.client.getContainingFile(className).then(
          (data) => {
            resolve({
              range:range,
              callback: () => atom.workspace.open(data.path, {initialLine: data.line})
            })
          },
          (xhr, status, error) => {
            resolve({
              range: range,
              callback: () => atom.notifications.addWarning("Can't go to anywhere :-(", {detail:`can't find source for ${className}`})
            })
          })
      } else resolve({
        range: range,
        callback: () => atom.notifications.addWarning("Can't go to anywhere :-(", {detail:`Couldn't find definition of ${text}`})
      })
    })
  }
}
