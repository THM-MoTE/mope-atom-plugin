...
export default class MoieClient {
  constructor(servername, port) {
    ...
    this.moveUrl = this.baseAddress + "ensemble/move"
  ...
  getContainingFile(file, position, word) {
    const json = JSON.stringify({
      file: file,
      position: position,
      word: word
  ...
  openMove(path, onSuccess, onError) {
    const json = JSON.stringify({
      path: path
    })
    if(this.connected) {
  ...
  recentProjects() {
    return jQuery.getJSON(this.baseAddress + `recent-files`)
  }
}
