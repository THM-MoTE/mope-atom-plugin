...
export default {
  ...
  getConfigPath() {
    return electron.remote.app.getPath("home") + "/.config/mope/mope.conf"
  },
  getLogPath() {
    return electron.remote.app.getPath("home") + "/.config/mope/mope-server.log"
  },
  openConfig() {
  ...
  startMoieServer(afterStart) {
    ...
    const command = atom.config.get('MoPE.javaExec')
      const args = ['-jar', atom.config.get('MoPE.mopeExec'),
                    '--compiler', atom.config.get('MoPE.compilerExec'),
                    '--port', port]
    const stdout = data => {
    ...
    const exit = code => {
        const logFileHint = "Set the path's in the plugin configuration for:\n" +
              "- Modelica compiler at 'compilerExec'\n"+
              "- Java runtime at 'javaExec'\n"+
              `(check the logfile at: ${this.getLogPath()} to see what is wrong.)`
      const options = {
        dismissable: false
      }
      switch(code) {
      ...
      console.log("Server started: ", command, args)
  },
  ...
  wordBelowCursor() {
    ...
    return (buffer.isEmpty()) ?
      undefined :
      this.wordBelowPosition(this.cursorPosition(), buffer)
  },
  ...
  wordBefore(pos, textBuffer) {
    ...
    while(!word.startsWith(" ") && startPos.column !== 0 && startPos.column !== -1) {
