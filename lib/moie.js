...
export default {
      ...
      (data) => {
        ...
        if(lines.length != 0) {
          data.forEach(errorObj => {
            const errorPoint = [errorObj.line-1,errorObj.column]
  ...
  createErrorLine(errorObj) {
    ...
    <span class="error-badge">Error</span> in line
    <span class="bold">${errorObj.line}</span> column
    <span class="bold">${errorObj.column}</span>: ${errorObj.message}</span>`
  },
