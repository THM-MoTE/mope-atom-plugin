...
import * as serverInfos from './mope-server'
...
export default {
  ...
  downloadServer() {
    ...
    const targetFile = `${homeDir}/.mope/mope-server-${serverInfos.serverVersion()}.jar`
    const fileObj = new File(targetFile)
    console.log("path ", targetFile, " obj ",fileObj)
    const megabyte = 1000000

    if(fileObj.existsSync() && fileObj.isFile()) {
      atom.notifications.addSuccess("Download finished", {detail: `Server already available at: ${targetFile}`})
    } else {
      const progressHandler = percent => {
        if (typeof(percent) !== "number") {
          atom.notifications.addError("Error downloading server",
            {detail:"An error occured while  downloading the server, try a manual download instead."})
          return
        }
        else if(percent === 1)
          atom.notifications.addSuccess("Server download finished")

        self.statusView.updateText(`${(percent*100).toFixed(2)}%`)
      }

      const downloadManager = new DownloadManager(24.3*megabyte, progressHandler)
      atom.notifications.addInfo("Download server", {detail: `Starting download of mope-server-${serverInfos.serverVersion()}.jar`})
      downloadManager.downloadFile(serverInfos.releaseUri(), targetFile)
    }
  },
