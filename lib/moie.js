...
export default {
  ...
  activate(state) {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      ...
      'moie:run-Script': () => this.compileScript(),
      'moie:stop-Server': () => this.stopServer(),
      'moie:check-model': () => this.checkModel(),
      'moie:open-documentation': () => this.openDoc(),
      'moie:display-comment': () => this.displayComment(),
      'moie:show-type': () => this.displayType(),
      'moie:generate-project.json': () => this.generateProjectConfig(),
      'moie:open-server-config': () => this.openServerConfig(),
      'moie:open-server-log': () => this.openServerLog()
    }))
  ...
  displayComment() {
    const word = util.wordBelowCursor()
    this.client.getComment(word,
      (data) => {
        const commentLine = `<span class="error-line icon icon-comment">
          <span class="info-badge">Comment</span>
          ${word} <span class="italic">${data.comment}</span>
          </span>`
        this.infoView.addLines([commentLine])
        this.infoPanel.show()
      },
      () => {
        atom.notifications.addWarning("Can't find comment :-(", {detail:`can't find comment of ${word}`})
      })
  },
