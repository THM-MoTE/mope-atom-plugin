...
export default {
  ...
  activate(state) {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      ...
      'moie:complete-word': () => this.completion()
    }))
  ...
  completion() {
    ...
    const bufferPosition = atom.workspace.getActiveTextEditor().getCursorBufferPosition()
    const position = new util.FilePosition(bufferPosition.row, bufferPosition.column)
    if(util.isModelicaFile(filepath)) {
      this.client.codeCompletion(filepath, position, word, (data) => {
      ...
      atom.notifications.addError("Can't give completions for non-modelica files")
    }
  },
