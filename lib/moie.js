...
export default {
  ...
  connect(projectDir) {
    ...
    const self = this
    const projectPromise = (projectDir === undefined) ?
                              this.selectProject(atom.project.getDirectories())
                              : Promise.resolve(projectDir)
    projectPromise.then(this.readProjectFileContent.bind(this))
      //.then(projJson => self.downloadServer().then(serverFile => projJson))
      ...
      .then(() => {
        ...
        if(atom.config.get('MoPE.startServer') && !this.triedAsSubprocess) {
          util.startMoieServer(() =>
            projectPromise.then(proj => self.connect(proj))
          )
  ...
  generateProjectConfig(projectDir) {
    ...
    return  projectPromise.then(projectDir => {
        ...
        if (!file.existsSync()) {
          ...
          return file.write(JSON.stringify(content, undefined, 2)).then(() => content)
        } else {
          return file.read()
        }
      })
  ...
  downloadServer() {
    ...
    const manager = new ServerDownloader(progressHandler)
    atom.notifications.addInfo("Download server", {detail: `Starting download of mope-server-${ServerDownloader.serverVersion()}.jar`})
    return manager.download()
    .then(file => {
      atom.notifications.addSuccess("Download completed.", {detail:
      `Server now available at: ${file.getPath()}`})
      return file
    })
  },
